...
import { deleteAttachment, downloadAttachment } from "../../services/productService";
    ...
    <Dialog open={open} onClose={loading ? undefined : onClose} fullWidth maxWidth="sm">
      ...
      <DialogContent dividers>
        ...
        <Button component="label" variant="outlined" startIcon={<AttachFileIcon />} disabled={loading}>
          ...
          <input hidden type="file" accept="application/pdf,image/jpeg,image/png,image/webp" onChange={(e) => e.target.files?.[0] && upload(e.target.files[0])} />
        </Button>
              ...
              secondaryAction={(
                <>
                  <Tooltip title="Baixar">
                    <IconButton onClick={() => downloadAttachment(file.id, file.fileName)}>
                      <DownloadIcon />
                    </IconButton>
                  </Tooltip>
                  <Tooltip title="Excluir">
                    <IconButton color="error" onClick={() => remove(file.id)}>
                      <DeleteOutlineIcon />
                    </IconButton>
                  </Tooltip>
                </>
